    ...
    <div class="col-xl-12">
        <div class="card m-b-30">
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-lg-12">
                        <a href="{{route('kir.users.create')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah</a>
                        <a href="#" class="btn btn-success btn-filter"><i class="fa fa-filter"></i> Fiter</a>
                        <a href="{{route('kir.users.index')}}" class="btn btn-warning"><i class="fa fa-refresh"></i> Refresh</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex mb-1">
                                                <a href="{{route('kir.users.show',$row->id)}}" class="btn btn-success btn-sm mr-1"><i class="fa fa-address-card"></i> Detail</a>
                                                <a href="{{route('kir.users.edit',$row->id)}}" class="btn btn-primary btn-sm mr-1"><i class="fa fa-edit"></i> Edit</a>
                                                @if(Auth::user()->id != $row->id)
                                                    @if(!$row->trashed())
                                                    <a href="#" class="btn btn-danger btn-sm mr-1 btn-delete" data-id="{{$row->id}}"><i class="fa fa-trash"></i> Hapus</a>
                                                        @if(Auth::user()->isSuperAdmin())
                                                            <a href="{{route('kir.users.impersonate',$row->id)}}" class="btn btn-warning mr-1"><i class="fa fa-sign-in"></i> Impersonate</a>
                                                        @endif
                                                    @else
                                                        @if(Auth::user()->isSuperAdmin())
...
@include("kir.pages.users.modal.index")
    ...
    $(function(){
        ...
        $(document).on("click",".btn-delete",function(){
            ...
            if(confirm("Apakah anda yakin ingin menghapus data ini ?")){
                $("#frmDelete").attr("action", "{{ route('kir.users.destroy', '_id_') }}".replace("_id_", id));
        ...
        $(document).on("click",".btn-restore",function(){
            ...
            if(confirm("Apakah anda yakin ingin mengembalikan data ini ?")){
                $("#frmRestore").attr("action", "{{ route('kir.users.restore', '_id_') }}".replace("_id_", id));
