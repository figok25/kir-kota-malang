                ...
                <div class="text-center d-flex flex-column h-100">
                    ...
                    <div class="mt-3 d-flex justify-content-between flex-column flex-1">
                        ...
                        <div class="d-flex justify-content-center mt-3">
                            <a href="{{route('kir.users.index')}}" class="btn btn-warning btn-sm" style="margin-right: 10px;"><i class="fa fa-arrow-left"></i> Kembali</a>
                            <a href="{{route('kir.users.edit',$result->id)}}" class="btn btn-primary btn-sm" style="margin-right: 10px;"><i class="fa fa-edit"></i> Edit</a>
    ...
    $(function(){
        ...
        $(document).on("click",".btn-delete",function(){
            if(confirm("Apakah anda yakin ingin menghapus data ini ?")){
                $("#frmDelete").attr("action", "{{ route('kir.users.destroy', '_id_') }}".replace("_id_", '{{$result->id}}'));
                $("#frmDelete").submit();
            }
        })
        ...
        $(document).on("click",".btn-restore",function(){
            if(confirm("Apakah anda yakin ingin mengembalikan data ini ?")){
                $("#frmRestore").attr("action", "{{ route('kir.users.restore', '_id_') }}".replace("_id_", '{{$result->id}}'));
                $("#frmRestore").submit();
            }
        })
    })
